...
if (isset($_POST['height'],$_POST['weight'],$_POST['health'])) {
    $height = (int)$_POST['height'];
    $weight = (int)$_POST['weight'];
    $health = trim($_POST['health']);

    if($height<160){
         header("Location: ../View/page.php?status=underheight");
         exit;
    }
    ...
    if($height>=160 && $height<=170)
    {
        if(($weight>=0 && $weight<=45) || $weight>110){
            header("Location: ../View/page.php?status=weightproblem");
            exit;
        }
        ...
        $stmt = $conn->prepare("UPDATE details SET height = ?, weight = ?, health = ?
            WHERE username = ?");
        $stmt->bind_param("iiss",$height, $weight,$health,$user);
        $stmt->execute();
        $stmt->close();
