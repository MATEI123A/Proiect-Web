...
if (isset($_POST['username'], $_POST['email'], $_POST['password'])) {
    ...
    if ($check_user->num_rows == 0 && $check_email->num_rows == 0) {
        ...
        $stmt->bind_param("sssi", $user, $email, $password, $status);
        $stmt->execute();
        $stmt->close();
        $check_user->close();
        $check_email->close(); 
        ...
        $stmt = $conn->prepare("INSERT INTO user_points (username,total_points,abs_points,legs_points,push_points,pull_points)
                                VALUES (?,?,?, ?, ?, ?)");
        $stmt->bind_param("siiiii", $user,$total_points,$abs_points,$legs_points,$push_points,$pull_points);
        $stmt->execute();
        $stmt->close();

        header("Location: ../View/mainpage.php?status=valid");
     
    } else header("Location: ../View/mainpage.php?status=invalid");
}
