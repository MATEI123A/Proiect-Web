...
<div class="top-buttons">
	...
	<button onclick="admin()" id="admin" class="green-hover">ADMIN</button>
...
<script>
	...
	function clasament() {
        fetch('../View/clasament.html?v='+ Date.now())
			...
			document.getElementById('admin').classList.remove("button-on-click");
	...
	function admin() {
		document.getElementById('admin').classList.add("button-on-click");
		document.getElementById('clasament').classList.remove("button-on-click");
		document.getElementById('exercices').classList.remove("button-on-click");
		document.getElementById('account').classList.remove("button-on-click");
		document.getElementById('yourdata').classList.remove("button-on-click");
		...
		paragraph.textContent = "List of all active users:";
		...
		.then(users => {
			users.forEach(user => {
				...
				userButton.textContent = "Username : " + user;
				userButton.onclick = function() {
					administrate_user(userButton.id); 
				};
	...
	function administrate_user(username){
		document.getElementById("users").innerHTML="";
		document.cookie = "username=" + username + "; path=/";

		fetch('../View/admin_user.html?v=' + Date.now())
            .then(response => response.text())        
            .then(html => document.getElementById('mainpage').innerHTML = html); 
		
		get_details_account_admin();
	}
	...
	function get_details_account_admin(){
        fetch('../Controller/get_data_admin.php')
            .then(response => response.json())
            .then(data => {
                document.getElementById("weight").value = data.weight;
				document.getElementById("height").value = data.height;
	...
	function delete_account(){
		fetch('../Model/delete_user.php?v=' + Date.now())
		admin();	
	}

	function edit(){
		 fetch('update_data_admin.html?v='+ Date.now())
                .then(response => response.text())        
                .then(html => document.getElementById('mainpage').innerHTML = html); 
	}
	...
	function back_edit(){
		fetch('admin_user.html?v=' + Date.now())
            .then(response => response.text())        
            .then(html => document.getElementById('mainpage').innerHTML = html); 
			get_details_account_admin();
	}
